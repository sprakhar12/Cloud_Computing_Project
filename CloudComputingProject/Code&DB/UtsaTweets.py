...
cur = conn.cursor()
cur.execute("select * from utsavsmarshall")

# List Instantiate
myList = []
UTSA_List = ['#UTSA','#BirdsUp','#GoRunners', '#Marshall', '#HerdOn']
myList2 = []
...
df = pd.DataFrame(data=[i for i in List[1:,2]], columns=['tweets'])
...
plt.title('Sentiment Analysis UTSAvsMarshall Tweets')
plt.xticks(index + bar_width, (''))
minor_ticks = np.arange(0, 120, 20)
plt.yticks(minor_ticks)
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.savefig('UTSAvsMarshall.png')
plt.show()
